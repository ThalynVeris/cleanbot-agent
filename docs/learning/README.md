...
本目录记录围绕 CleanBot 的「边学边做」计划：每个单元由 Claude 提供完整代码与讲义，学习者负责精读、对照手敲核心代码、用 AI 修改并审查代码，最终把项目推进为面向企业售后场景的 Agent 平台。
...
|---|---|
| 🟢 复制 | 粘贴运行，能一句话讲清作用 |
| 🟡 精读 | 逐行读懂；回答讲义中的「为什么」；用 AI 改一处并按清单审查 diff |
...
每个单元交付：完整代码 + 测试 + `Mx.y.md` 讲义。完成标准：测试通过、问题已作答（写在讲义末尾的「我的回答」）、🔴 已手敲。

### M0 吃透现有代码（P0）
- [x] [M0.1](M0.1.md) 修复 Rerank 默认策略与文档不一致；建立学习体系
- [ ] [M0.2](M0.2.md) 检索链路精读：Dense、BM25、RRF、条件 Rerank
